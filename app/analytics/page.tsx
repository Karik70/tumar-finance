...
export default function AnalyticsPage() {
  ...
  async function load(s:any,months:number){
    ...
    // Monthly aggregation
    const map: Record<string,{income:number,bank_expense:number,cash_expense:number}> = {}
    const now = new Date()
    for(let i=months-1;i>=0;i--){
      const d=new Date(now.getFullYear(),now.getMonth()-i,1)
      const k=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`
      map[k]={income:0,bank_expense:0,cash_expense:0}
    }
    bank.forEach((r:any)=>{
      const k=r.entry_date.slice(0,7)
      if(!map[k]) return
      if(r.type==='income') map[k].income+=Number(r.amount)
      else map[k].bank_expense+=Number(r.amount)
    })
    cash.forEach((r:any)=>{
      const k=r.entry_date.slice(0,7)
      if(!map[k]) return
      if(r.type==='expense') map[k].cash_expense+=Number(r.amount)
    })
    const monthly = Object.entries(map).map(([k,v])=>({
      month: MONTHS[parseInt(k.slice(5,7))-1],
      income: Math.round(v.income),
      expense: Math.round(v.bank_expense+v.cash_expense),
      profit: Math.round(v.income-v.bank_expense-v.cash_expense)
    }))
    setMonthly(monthly)
    ...
    <div style={{display:'flex',minHeight:'100vh'}}>
      ...
      <main style={{flex:1,padding:'28px 32px',minWidth:0}}>
        ...
        <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:12,padding:'20px 24px',marginBottom:20}}>
          <div style={{fontSize:14,fontWeight:500,marginBottom:16}}>Доходы vs расходы по месяцам</div>
        ...
        <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:12,padding:'20px 24px'}}>
          <div style={{fontSize:14,fontWeight:500,marginBottom:16}}>Чистый результат по месяцам</div>
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={monthly}>
              ...
              <YAxis tick={{fontSize:11,fill:'var(--text2)'}} axisLine={false} tickLine={false} tickFormatter={v=>v>=1000000?(v/1000000).toFixed(1)+'M':(v/1000).toFixed(0)+'K'} />
              <Tooltip {...tooltip} />
              <Line type="monotone" dataKey="profit" stroke="#58a6ff" strokeWidth={2.5} dot={(props:any)=>{const c=props.value>=0?'#3fb950':'#f85149';return<circle key={props.index} cx={props.cx} cy={props.cy} r={4} fill={c} stroke={c}/>}} name="Результат" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </main>
